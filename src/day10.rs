...
fn start_up(grid: &Grid<Tile>, idx: (usize, usize)) -> Option<Direction> {
    if idx.0 != 0 {
        match grid[(idx.0 - 1, idx.1)] {
            Vert | SW | SE => Some(Up),
            _ => None,
        }
    } else {
...
fn start_down(grid: &Grid<Tile>, idx: (usize, usize)) -> Option<Direction> {
    if idx.0 + 1 != grid.n_rows {
        match grid[(idx.0 + 1, idx.1)] {
            Vert | NW | NE => Some(Down),
            _ => None,
        }
    } else {
        None
...
fn start_left(grid: &Grid<Tile>, idx: (usize, usize)) -> Option<Direction> {
    if idx.1 != 0 {
        match grid[(idx.0 - 1, idx.1)] {
            Horz | NE | SE => Some(Left),
            _ => None,
        }
    } else {
        None
...
fn start_right(grid: &Grid<Tile>, idx: (usize, usize)) -> Option<Direction> {
    if idx.1 + 1 != grid.n_cols {
        match grid[(idx.0, idx.1 + 1)] {
            Horz | NW | SW => Some(Right),
            _ => None,
        }
    } else {
        None
...
impl FromStr for Maze {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let dir = start_up(&grid, start)
            ...
            .or_else(|| start_left(&grid, start))
            .or_else(|| start_right(&grid, start))
            .ok_or_else(|| s.to_string())?;
        Ok(Maze { grid, start, dir })
    }
}
...
impl Tile {
    ...
    pub fn mainloop_redirect(&self, visitor_dir: Direction) -> Option<Direction> {
        match (self, visitor_dir) {
            (Vert, x @ (Up | Down)) => Some(x),
            (Horz, x @ (Left | Right)) => Some(x),
            (NE, Down) => Some(Right),
            (NE, Left) => Some(Up),
            (NW, Down) => Some(Left),
            (NW, Right) => Some(Up),
...
impl Visitor2<'_> {
    ...
    pub fn advance(&mut self) -> bool {
        match self.grid[self.current].mainloop_redirect(self.dir) {
...
impl<'a> From<&'a Maze> for Visitor2<'a> {
    fn from(maze: &'a Maze) -> Self {
        Self {
            current: maze.start.clone(),
            steps: 0,
            dir: maze.dir.clone(),
            grid: &maze.grid,
        }
...
impl<'a> fmt::Display for EscapeAnalyzer<'a> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let (n_rows, n_cols) = self.maze.grid.shape();
        for i in 0..n_rows {
            for j in 0..n_cols {
                match self.states.borrow()[(i, j)] {
                    Inside => write!(f, "I")?,
                    Outside => write!(f, "O")?,
                    _ => write!(f, "{}", self.maze.grid[(i, j)])?,
                }
            }
            if i != n_rows - 1 {
                write!(f, "\n")?;
...
pub struct OutsideExpander {
    current: (usize, usize),
    states: Rc<RefCell<Grid<State>>>,
}
impl OutsideExpander {
    pub fn move_up(&mut self) -> bool {
        if self.current.0 != 0 {
            self.current.0 -= 1;
            true
    ...
    pub fn move_down(&mut self) -> bool {
        let new = self.current.0 + 1;
        if new < self.states.borrow().n_rows {
            self.current.0 = new;
            true
        } else {
            false
    ...
    pub fn move_right(&mut self) -> bool {
        let new = self.current.1 + 1;
        if new < self.states.borrow().n_cols {
            self.current.1 = new;
            true
        } else {
            false
    ...
    pub fn expand(&mut self) -> bool {
        let state = self.states.borrow()[self.current].clone();
        match state {
            Null => {
                self.states.borrow_mut()[self.current] = Outside;
...
LJ...";
    ...
    fn traverse() {
        ...
        let maze = TEST1.parse::<Maze>().unwrap();
        assert_eq!(maze.farthest(), 4);
        let maze = TEST2.parse::<Maze>().unwrap();
